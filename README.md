...
### 6c. Registering the slash command in production

**You don't need to do anything for this.** The bot registers its own
`/gif` command automatically every time it starts up (see
`registerCommands()` in `src/index.ts`) — this works even on Render's
free tier, which has no Shell access. Just:

- **Fastest for testing:** set `DISCORD_GUILD_ID` in Render's env vars to
  your test server's ID. The command registers there within seconds of
  the bot booting.
- **For every server the bot is in:** leave `DISCORD_GUILD_ID` empty.
  Global commands can take **up to ~1 hour** to show up everywhere the
  first time (Discord's own propagation delay, not something the bot
  controls).

If you ever see a "Required option ... not found" error, it means Discord
or your Discord client is holding onto a stale command definition (most
often from switching between guild-scoped and global at some point).
Restart the Render service to force re-registration, and reload your
Discord client (`Ctrl+R`, or fully close/reopen on mobile) to clear its
local cache. If that's not enough, run `npm run clear-commands` from your
own computer (with `.env` filled in) to wipe every registration, then
restart the bot on Render to re-register cleanly.
...
- **Re-register commands after changing `src/commands/gif.ts`:** nothing
  to do — it registers automatically on the next boot, which happens
  every time you push (see 6c). If you're impatient, restart the service
  manually.
